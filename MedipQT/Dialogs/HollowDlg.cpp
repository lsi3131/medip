...
HollowDlg::HollowDlg(DataContext* pDataContext, QWidget *parent /*= NULL*/)
	: MeshBaseDlg(pDataContext, parent),
	m_dOffsetDistance(-2.0),
	m_nEdgeLength(0.0),
	m_nState(0)
{
	setWindowTitle(QString("Hollow"));
	initQtUI();
}

HollowDlg::HollowDlg(DataContext* pDataContext, QString _title, QWidget *parent /*= NULL*/)
...
void HollowDlg::closeEvent(QCloseEvent *)
{
...
void HollowDlg::reject(bool bForce)
{
	if (!m_pDataContext || !ACTION_MANAGER->isActionFinished() && !bForce) return; //20210928_byPHS_기능수행중 Reject 안되게 수정
	...
	ACTION_MANAGER->action_UndoRedo_update(m_pDataContext, MESH_WORK_NONE, MESH_WORK_HOLLOW);
...
void HollowDlg::initQtUI()
{
	...
	//
	QLabel *lb_H_0 = new QLabel(QString("Hollow"), this);
	layoutH_0->addWidget(lb_H_0);
	lb_H_0->setStyleSheet("background-color: #414141;");

	//Offset
	double min = -4.0; double max = 4.0; int demical = 2; double initVal = -2.0;
	...
	//EdgeLength
	min = 0.0; max = 1.0; demical = 2; initVal = 0.5;
	...
	connect(btnOK, &QPushButton::clicked, this, &HollowDlg::OnOK);
	connect(btnCancel, &QPushButton::clicked, this, &HollowDlg::OnCancel);	
	connect(LE_Size_H_2_0, &QLineEdit::editingFinished, this, &HollowDlg::OnTextChanged);
	connect(LE_Size_H_2_0, &QLineEdit::returnPressed, this, &HollowDlg::OnTextChanged);
	connect(sSlider0.pSlidier, &QSlider::valueChanged, this, &HollowDlg::OnDepthSlideReleased);
	connect(LE_Size_H_2_1, &QLineEdit::editingFinished, this, &HollowDlg::OnTextChanged);
	connect(LE_Size_H_2_1, &QLineEdit::returnPressed, this, &HollowDlg::OnTextChanged);
	connect(sSlider1.pSlidier, &QSlider::valueChanged, this, &HollowDlg::OnDepthSlideReleased);
	connect(btnUpdate, &QPushButton::clicked, this, &HollowDlg::OnUpdateHollow);
...
double HollowDlg::calcEdgeLength(mip::MeshTopology* pMT)
{
	return 0.0f;
}

void HollowDlg::exitDialog()
{
...
void HollowDlg::OnDepthSlideReleased(int val)
{
	double tmpVal = val;
...
void HollowDlg::OnComboChanged(int index)
{
...
void HollowDlg::OnUpdateHollow()
{
	...
	WorkMeshRemesh* pWMR = ACTION_MANAGER->action_Mesh_Init(MESH_HOLLOW, MeshIdx, m_pDataContext, pViewer);
...
void HollowDlg::OnOK()
{
	...
	setState(enHollowState::HOLLOW_APPLY);
	ACTION_MANAGER->action_UndoRedo_update(m_pDataContext, MESH_WORK_NONE, MESH_WORK_MODE::MESH_WORK_HOLLOW);
