#ifndef __NRRD_RESAMPLE_HXX
#define __NRRD_RESAMPLE_HXX

//#include <NRRD/GetSet/StringUtil.hxx>
//#include <NRRD/GetSet/GetSetAutoGui.hxx>
//#include <NRRD/GetSet/GetSetCmdLine.hxx>
...
//void resample(const std::string& file)
{
	// Get output name
//	std::string basename = ("File/Output");
	std::string basename = files[0];
	std::string extension = splitRight(basename, ".");
	// Sampling
//	int    bx = (int)("Samplig/Binning/Binsize X");
	int    bx = (int)(1);
//	int    by = (int)("Samplig/Binning/Binsize Y");
	int    by = (int)(1);
//	int    bz = (int)("Samplig/Binning/Binsize Z");
	int    bz = (int)(1);
	int    mode = (int)("Samplig/Intensity");
	double bias = (double)(0);// "Samplig/Intensity Bias");
	double scale = (double)(1.0);// "Samplig/Intensity Scale");
	bool   clamp = (bool)("Samplig/Clamping/Enable");
	double clamp_min = (double)(0);// "Samplig/Clamping/Min");
	double clamp_max = (double)(0);// "Samplig/Clamping/Max");
	bool   removeNaN = (bool)("Samplig/Set NaN to zero");
	int    fsk = (int)("Samplig/Frameskip");
	int    order = (int)("Samplig/Directions");
//	bool   flipx = (bool)("Sampling/Flip Output X");
	bool   flipx = (bool)(false);
//	bool   flipy = (bool)("Sampling/Flip Output Y");
	bool   flipy = (bool)(false);
//	bool   flipz = (bool)("Sampling/Flip Output Z");
	bool   flipz = (bool)(false);
...
//			GetSetAutoGUI::error("File Access Error", files[i]);
			QMessageBox::warning(NULL, "Warning", "File Access Error");
		}
		int insize0 = in.size(0);
		int insize1 = in.size(1);
		int insize2 = in.size(2);		
		int od = in.size(2)*bz;
//		int od = bz;
		if (d == 2) {
			bz = 1;
		...
		int outsize0 = out.size(0);
		int outsize1 = out.size(1);
		int outsize2 = out.size(2);
		double sumv = 0;
		double sumvv = 0;
		double minv = in[0];
		double maxv = in[0];
...
//#include "NRRD/GetSet/BaseTypes.hxx"
