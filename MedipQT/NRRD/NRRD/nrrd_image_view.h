...
//		double operator()(double x, double y, double z=0) const
		double interpolation(double x, double y, double z = 0) const
		{
		...
		T2* convert_type(T1 *pSource, int size)
		{
			return convert<T1, T2>(pSource, size);
		}
		...
		bool save(VOLUME_DATA* pVolumeData, const std::string& nrrd_file, double scaleX, double scaleY, double scaleZ) const
		{
			nrrd_header["spacings"] = vectorToString<double>(element_spacing, " ");		
			return !!(*this) && NRRD::save<T>(pVolumeData, scaleX, scaleY, scaleZ, nrrd_file, data, (int)dim.size(), &(dim[0]), meta_info, nrrd_header);
		}
...
//			std::string file = "file.nrrd";
		...
		/// Save an image as a .nrrd file.
		void resample(const std::vector<std::string>& files) const
		{			
			NRRD::resample<T, T>(files);
		}
		/*void resample(const std::string& file) const
		{
			NRRD::resample<T, T>(file);
		}*/
