...
//			tgt[i] = (T)(src[i] / 257.0);
	...
	template <typename T>
	bool save(VOLUME_DATA* pVolumeData, const std::string& file, T* data, int n, const int *size,
		std::map<std::string, std::string> hdr_keys = std::map<std::string, std::string>(),
		std::map<std::string, std::string> hdr_fields = std::map<std::string, std::string>())
	{
		...
		nrrd << "endian: big\n";
		nrrd << "dimension: 3\n";
		nrrd << "sizes: 256 256 179\n";
		nrrd << "space dimension: 3\n";
		nrrd << "space directions: (1.472,0,0) (0,1.472,0) (0,0,3.0)\n";*/
		nrrd << "space units: \"mm\" \"mm\" \"mm\"\n";

		// Type. special case: char maps to int8.
//		hdr_fields["type"] = typeName<char>();
//		hdr_fields["type"] = typeName<short>();
		hdr_fields["type"] = typeName<T>();
		if ((hdr_fields["type"] == "char") || (hdr_fields["type"] == "unsigned char"))
			hdr_fields["type"] = "uint8";
		else if (hdr_fields["type"] == "unsigned" || hdr_fields["type"] == "unsigned int")
			hdr_fields["type"] = std::string("uint") + toString(sizeof(char) * 8);
		else if (hdr_fields["type"] == "short")
		{
			//		hdr_fields["type"] = std::string("uint") + toString(sizeof(unsigned char) * 16);
			hdr_fields["type"] = "short";
		}
		// Dimension and size
		hdr_fields["dimension"] = toString(n);
		hdr_fields["space dimension"] = toString(n);
		//	nrrd << "space directions: (1.472,0,0) (0,1.472,0) (0,0,3.0)\n";
		//	nrrd << "space directions: (3.0,0,0) (0,3.0,0) (0,0,3.0)\n";
		VOLUME_DATA *volume_data = pVolumeData;
		/*float getImgPosX() { return imgPos.x; }
		float getImgPosY() { return imgPos.y; }
		float getImgPosZ()*/
		nrrd << "space directions: (" + toString(volume_data->getSpaceX(true)) + ",0,0) (0," + toString(volume_data->getSpaceY(true)) + ",0) (0,0," + toString(volume_data->getSpaceZ(true)) + ")\n";
		...
		for (auto it = hdr_keys.begin(); it != hdr_keys.end(); ++it)
			...
			//	nrrd << to_string(offset, 8, ' ') + raw_read_info_end;
			int ne = 1;
			for (int i = 0; i<n; i++) ne *= size[i];
			//	nrrd.write((char*)data, sizeof(T)*ne);
			//	nrrd.write((char*)data, sizeof(int)*ne);
//			unsigned char *pTargetData = convert<short, unsigned char>(data, ne);
			nrrd.write((char*)data, sizeof(unsigned char)*ne);
			return true;
	}

	template <typename T>
	bool save(VOLUME_DATA* pVolumeData, float scaleX, float scaleY, float scaleZ, const std::string& file, T* data, int n, const int *size,
		std::map<std::string, std::string> hdr_keys = std::map<std::string, std::string>(),
		std::map<std::string, std::string> hdr_fields = std::map<std::string, std::string>())
	{
		std::ofstream nrrd(file.c_str(), std::ios::binary);
		...
		float getImgPosY() { return imgPos.y; }
		float getImgPosZ()*/
		//		nrrd << "space directions: (" + toString(volume_data->getSpaceX(true)*2.0) + ",0,0) (0," + toString(volume_data->getSpaceY(true)*2.0) + ",0) (0,0," + toString(volume_data->getSpaceZ(true)*2.0) + ")\n";
		nrrd << "space directions: (" + toString(volume_data->getSpaceX(true) / scaleX) + ",0,0) (0," + toString(volume_data->getSpaceY(true) / scaleY) + ",0) (0,0," + toString(volume_data->getSpaceZ(true) / scaleZ) + ")\n";
		hdr_fields["sizes"] = toString(size[0]);
		for (int i = 1; i<n; i++)
			hdr_fields["sizes"] = hdr_fields["sizes"] + " " + toString(size[i]);
		// Encoding
