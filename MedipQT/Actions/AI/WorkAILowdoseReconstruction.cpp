#include "stdafx.h"
#include "WorkAILowdoseReconstruction.h"
...
WorkAILowdoseReconstruction::WorkAILowdoseReconstruction(VOLUME_DATA* pVolumeData, ActionManager* pActionManager) :
	...
	m_pActionManager(pActionManager)
{
	...
	m_AIWeightFileName = QString("CT_Dose_QD.mipx");
}

void WorkAILowdoseReconstruction::threadRun()
{
	...
	InsertActionThreadResult(m_pActionManager, ACTP_AI_LOWDOSE_RECONSTRUCTION_PREDICT, m_OutputData);

	emit finished();
}

bool WorkAILowdoseReconstruction::doPredict(int addValue)
{
	...
	{
		qWarning() << "fail to WorkAILowdoseReconstruction. result empty";
		return false;
	}
