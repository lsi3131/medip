#include "stdafx.h"
#include "WorkAICTQuantization.h"
...
WorkAICTQuantization::WorkAICTQuantization(VOLUME_DATA* pVolumeData, ActionManager* pActionManager) :
	...
	m_pActionManager(pActionManager)
{
	...
	m_AIWeightFileName = QString("CT_Quant_PDFF.mipx");
}

WorkAICTQuantization::~WorkAICTQuantization()
{
	qInfo() << "destructor WorkAICTQuantization";
}

void WorkAICTQuantization::threadRun()
{
	...
	InsertActionThreadResult(m_pActionManager, ACTP_AI_CT_QUANTIZATION_PREDICT, m_OutputData);

	emit finished();
}

bool WorkAICTQuantization::doPredict(int addValue)
{
