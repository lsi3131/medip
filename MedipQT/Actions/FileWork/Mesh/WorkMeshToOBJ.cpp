...
WorkMeshToOBJ::WorkMeshToOBJ(VOLUME_DATA* pVolumeData, QString& strFilename, mip::MeshTopology* m, float sc, bool bPatientCoordinate) :
	m_objFilepath(strFilename),
	_mesh(m),
	_scale(sc),
...
void WorkMeshToOBJ::setProgressValue(int value, bool init)
{
	static int val = 0;
...
void WorkMeshToOBJ::upateprogress(vtkObject*, unsigned long eid, void* clientdata, void* calldata)
{
	WorkMeshToOBJ* worker = (WorkMeshToOBJ*)clientdata;
	if (worker)
	{
...
void WorkMeshToOBJ::threadRun()
{
	...
	if (m_bPatientCoordinate)
	{
		...
		double space[3];
		space[0] = m_pVolumeData->getSpaceX();
		space[1] = m_pVolumeData->getSpaceY();
		space[2] = m_pVolumeData->getSpaceZ();
		...
		patientMat.m[2][3] -= zdir[2] * space[2] * (nSlice - 1);
	...
	vtkSmartPointer<vtkLODActor> skinActor = vtkSmartPointer<vtkLODActor>::New();
	...
	m_objFilepath.replace(".obj", "");
	m_objFilepath.replace(".OBJ", "");

	vtkSmartPointer<vtkOBJExporter> objWriter = vtkSmartPointer<vtkOBJExporter>::New();
	objWriter->SetFilePrefix(m_objFilepath.toLocal8Bit().constData());
	objWriter->SetRenderWindow(renderWindow);
	objWriter->Write();

	setProgressValue(100);

	QString mtlFilepath = m_objFilepath + ".mtl";
	LPCWSTR strVariable = (const wchar_t*)mtlFilepath.utf16();
