...
#include "Actions/AI/WorkAICTQuantization.h"
#include "Test/TestContext.h"

class TestWorkAICTQuantization : public ::testing::Test
{
public:
	void SetUp() override
	{
...
#ifndef _DEBUG
TEST_F(TestWorkAICTQuantization, TestRunEmptyVolume)
{
	WorkAICTQuantization work(m_pVolumeData, m_pActionManager);

	work.threadRun();

	std::shared_ptr<std::vector<mint16>> pResult = PopActionThreadResult<std::vector<mint16>>(m_pActionManager, ACTP_AI_CT_QUANTIZATION_PREDICT);
	EXPECT_EQ(nullptr, pResult);
}

TEST_F(TestWorkAICTQuantization, TestRunSimple_BoundaryTest)
{
