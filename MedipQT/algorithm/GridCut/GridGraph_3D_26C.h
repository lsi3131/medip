...
public:
  ...
  void compute_maxflow(progUpdatefunc update = 0, void * data = 0);
...
template <typename type_tcap,typename type_ncap,typename type_flow>
void GridGraph_3D_26C<type_tcap,type_ncap,type_flow>::compute_maxflow(progUpdatefunc update, void * data)
{
  unsigned int size = (free_nodes.ifront - free_nodes.iback)/ sizeof(int);
  ...
  while(!free_nodes.empty())
  {
	...
	if (update)
	{
		unsigned int cnt = (free_nodes.ifront - free_nodes.iback) / sizeof(int);
		update((cnt - size) / size * 30 + 20, data);
	}
