...
namespace mip
{
	...
	public:
		...
		bool checkInside(const VECTOR3 & v) const
		{
			...
			return ((v.x > min.x) || (fabs(v.x - min.x) < MIP_EPSILON2))
				&& ((v.x < max.x) || (fabs(v.x - max.x) < MIP_EPSILON2))
				&& ((v.y > min.y) || (fabs(v.y - min.y) < MIP_EPSILON2))
				&& ((v.y < max.y) || (fabs(v.y - max.y) < MIP_EPSILON2))
				&& ((v.z > min.z) || (fabs(v.z - min.z) < MIP_EPSILON2))
				&& ((v.z < max.z) || (fabs(v.z - max.z) < MIP_EPSILON2));
		}
		...
		bool checkPoint(const VECTOR3 & v) const
		{
			...
			//	v.y >= min.y && v.y <= max.y &&
			//	v.z >= min.z && v.z <= max.z)
			if ( ((v.x > min.x) || (fabs(v.x - min.x) < MIP_EPSILON2)) 
				&& ((v.x < max.x) || (fabs(v.x - max.x) < MIP_EPSILON2))
				&& ((v.y > min.y) ||  (fabs(v.y - min.y) < MIP_EPSILON2))
				&& ((v.y < max.y) || (fabs(v.y - max.y) < MIP_EPSILON2))
				&& ((v.z > min.z) || (fabs(v.z - min.z) < MIP_EPSILON2))
