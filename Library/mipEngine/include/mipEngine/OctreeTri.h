...
//		void drawTest();
	...
	private:
		...
		bool sortNodeNew(OcNode* node, std::vector<int>& TrisList);
		unsigned int insideTrisList(AABB& aabb, std::vector<int> & TrisList, std::vector<int> & insideTrisList);
		bool isPolygonContainedNew(int& ttri_index, AABB& aabb);
	};
};