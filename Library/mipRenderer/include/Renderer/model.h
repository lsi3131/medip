...
namespace mip
{
	...
	namespace model
	{
		...
		*/
		...
		bool LoadSTLFileInMemory(const std::vector<mip::VECTOR3>& normals,  MeshTopology* mesh, float scale = 1.0f);

		bool IsSTLBinary(const char* filename);
		bool OpenBinary(const char* filename, int& loadMask, MeshTopology* mesh, float scale = 1.0f);
		bool OpenBinary_V2(const char* filename, int& loadMask, MeshTopology* mesh, float scale = 1.0f);
