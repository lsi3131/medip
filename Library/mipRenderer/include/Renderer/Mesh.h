...
#define VAO_ATTRIB_ID_TEX_DATA	5
#define VAO_ATTRIB_ID_USER_01	6
#define VAO_ATTRIB_ID_USER_02	7
#endif
...
namespace mip
{
	...
	public:
		...
		bool renderBegin(mip::Renderer* renderer, muint32 shaderProg = 0, bool backface_culling = false, bool b_depth_test = true);
		bool renderEnd(mip::Renderer* renderer, muint32 shaderProg = 0);
		bool renderPosition(const mip::MATRIX44* world, const mip::MATRIX44& view, const mip::MATRIX44& proj, const mip::COLOR* color = NULL, bool b_wire = false, bool vr_mode = false, const mip::MATRIX44* vr_view = NULL);
		bool renderPosition(ShaderCommon& _ShaderCommon, ShaderMeshData& _ShaderMeshData, int shaderMode = 0);
		...
		void setAlpha(float fAlphaVal); //0 ~ 255
		float getAlphaVal();
		void enableAlpha(bool bAlpha);

		ShaderMeshData* getShaderMeshData();
		ShaderCommon* getShaderCommon();
		ShaderMesh* getShaderMesh();

		void setShaderMeshData(ShaderMeshData _data);
		void setShaderMeshData(ShaderCommon _data);

		void setShaderType(muint32 nType);
		muint32 getShaderType();

		void glStatus(muint32 nShaderType);

		int initShader(muint32 nType);
		int createTexture(unsigned char* data, muint32 width, muint32 height, muint32 nrChannels, unsigned int type = 0);
		int addTextureId(muint32 textureId, const std::string& _strName);
		int deleteTexture();
		int getTextureCnt();
	...
	public:
		...
		std::vector<float> m_textureID;
		std::vector<muint32> m_textureIDList;
		std::vector<mip::TEXTURE> m_TextureDataList;

		std::vector<mip::VECTOR4> m_vertColors;
		mip::VECTOR4 m_baseColor;
	...
	protected:
		...
		muint32 m_colorBuffer;
		muint32 m_uvBuffer;
		muint32 m_textureIdBuffer;

		muint32 m_shaderProgram;
		muint32 m_shaderType;
		float m_fAlpha;
		ShaderCommon m_ShaderCommon;
		ShaderMeshData m_ShaderMeshData;
